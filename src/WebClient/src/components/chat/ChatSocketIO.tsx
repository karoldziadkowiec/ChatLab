...
const ChatSocketIO = () => {
    const METRICS_TOAST_ID = 'socketio-send-metrics';
    ...
    // Used for accurate latency measurement: correlate created message id with Socket.IO receive.
    ...
    const waitForReceive = useCallback((messageId: number, timeoutMs: number): Promise<void> => {
        ...
        return new Promise<void>((resolve, reject) => {
            const timeout = window.setTimeout(() => {
                ...
                reject(new Error('Timed out waiting for Socket.IO receive.'));
            }, timeoutMs);
    ...
    useEffect(() => {
        ...
        AccountService.getAuthorizationHeader()
            ...
            .then(() => {
                ...
                const onMessage = (incoming: Message) => {
                    if (typeof incoming?.id === 'number') {
                        receivedMessageIdsRef.current.add(incoming.id);
                        const waiter = receiveWaitersRef.current.get(incoming.id);
                        if (waiter) {
            ...
            .catch((err) => {
                ...
                const msg = err?.message ? String(err.message) : 'Failed to connect to Socket.IO.';
                if (msg.includes('Unauthorized join')) {
                    const details = chatData
                        ? ` (you=${userId ?? '?'}, chat.user1Id=${chatData.user1Id ?? '?'}, chat.user2Id=${chatData.user2Id ?? '?'})`
                        : '';
                    toast.error(`Socket.IO: Unauthorized join${details}. If this is load-testing, enable ALLOW_NON_PARTICIPANTS_IN_CHATS=true in SocketIoService.`);
                } else {
                    toast.error(err?.message ? `Socket.IO: ${err.message}` : 'Failed to connect to Socket.IO.');
                }
            });
    ...
    const sendMessageWithMetrics = useCallback(async (content: string, mode: 'manual' | 'auto') => {
        ...
        try {
            ...
            const created = await chatHubSocketIO.sendMessage(dto);
            if (!created || typeof created.id !== 'number') {
                throw new Error('Socket.IO send did not return created message with id.');
            }
            ...
            if (echoMs != null) {
                ...
                const toastMessage = `Socket.IO (${mode}): time ${echoRounded} ms | throughput ${throughput.toFixed(2)} msg/s`;
                showMetricsToast(toastMessage);
                console.log(`Socket.IO send metrics (${mode}): time=${echoRounded}ms, throughput=${throughput.toFixed(2)} msg/s`);
                return;
            }

            // Timeout path
            const throughput = getCurrentThroughputMsgPerSec();
            const toastMessage = `Socket.IO (${mode}): time timeout | throughput ${throughput.toFixed(2)} msg/s`;
            showMetricsToast(toastMessage);
        } catch (e) {
            if (isAuto) {
    ...
    }, [chatHubSocketIO, chatData?.id, userId, technologyId, resolveReceiverIdForSend, waitForReceive, getCurrentThroughputMsgPerSec, showMetricsToast]);
